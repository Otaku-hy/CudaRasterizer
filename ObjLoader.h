...
namespace objl
{
	...
	// Namespace: LoaderMath
	//
	// Description: The namespace that holds all of the LoaderMath
	...
	namespace algorithm
	{
		...
		{
			Vector3 cp1 = LoaderMath::CrossV3(b - a, p1 - a);
			Vector3 cp2 = LoaderMath::CrossV3(b - a, p2 - a);

			if (LoaderMath::DotV3(cp1, cp2) >= 0)
				return true;
			else
				return false;
		}
		...
		inline Vector3 GenTriNormal(Vector3 t1, Vector3 t2, Vector3 t3)
		{
			...
			Vector3 normal = LoaderMath::CrossV3(u, v);
		...
		inline bool inTriangle(Vector3 point, Vector3 tri1, Vector3 tri2, Vector3 tri3)
		{
			...
			Vector3 proj = LoaderMath::ProjV3(point, n);

			// If the distance from the triangle to the point is 0
			//	it lies on the triangle
			if (LoaderMath::MagnitudeV3(proj) == 0)
				return true;
			else
				return false;
		}
	...
	private:
			...
			std::string icurline)
		{
			...
			if (noNormal)
			{
				...
				Vector3 normal = LoaderMath::CrossV3(A, B);
			...
			const std::vector<Vertex>& iVerts)
		{
			...
			while (true)
			{
				...
				for (int i = 0; i < int(tVerts.size()); i++)
				{
					...
					float angle = LoaderMath::AngleBetweenV3(pPrev.Position - pCur.Position, pNext.Position - pCur.Position) * (180 / 3.14159265359);
